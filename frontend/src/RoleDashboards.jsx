...
function Table({columns, rows}) { return <div className="overflow"><table><thead><tr>{columns.map(c=><th key={c}>{c}</th>)}</tr></thead><tbody>{rows.length ? rows.map((row,i)=><tr key={row.id||i}>{row.map((cell,j)=><td key={j}>{cell ?? '—'}</td>)}</tr>) : <tr><td colSpan={columns.length} className="muted">No records found.</td></tr>}</tbody></table></div> }
function Stat({label,value,index,note='Live data'}){return <article className="stat-card"><span className={'icon '+['blue','green','orange','purple'][index]}><LayoutDashboard/></span><div><p>{label}</p><strong>{value ?? '—'}</strong><small>{note}</small></div></article>}

function NoticeForm({onCreated}) { const [form,setForm]=useState({title:'',description:'',category:'general'}),[busy,setBusy]=useState(false),[error,setError]=useState(''); const update=e=>setForm({...form,[e.target.name]:e.target.value});
  async function submit(e){e.preventDefault();setBusy(true);setError('');try{const {data}=await api.post('/notices',form);onCreated(data.notice);setForm({title:'',description:'',category:'general'});}catch(err){setError(message(err))}finally{setBusy(false)}}
  return <form className="workspace-form" onSubmit={submit}><input name="title" placeholder="Announcement title" value={form.title} onChange={update} required/><textarea name="description" placeholder="Details for students and staff" value={form.description} onChange={update} required/><select name="category" value={form.category} onChange={update}><option value="general">General</option><option value="academic">Academic</option><option value="class">Class</option></select>{error&&<p className="error">{error}</p>}<button disabled={busy}>{busy?'Publishing…':'Publish announcement'}</button></form> }


function AttendanceControl({subjects,onChanged}) { const [sessions,setSessions]=useState([]),[form,setForm]=useState({subjectId:'',section:'A',durationMinutes:8}),[error,setError]=useState(''),[busy,setBusy]=useState(false);
  const load=async()=>{try{const {data}=await api.get('/attendance-sessions/active');setSessions(data.sessions)}catch(e){setError(message(e))}}; useEffect(()=>{load()},[]);
  async function start(e){e.preventDefault();setBusy(true);setError('');try{await api.post('/attendance-sessions/start',{...form,durationMinutes:Number(form.durationMinutes)});await load();onChanged?.()}catch(err){setError(message(err))}finally{setBusy(false)}}
  async function stop(id){try{await api.post(`/attendance-sessions/${id}/stop`);await load();onChanged?.()}catch(err){setError(message(err))}}
...
function Workspace({role,title,data,student,canPublish,canSessions,reload}) { const timetable=data.timetable.map(x=>[day[x.day_of_week],`${formatTime(x.start_time)}–${formatTime(x.end_time)}`,x.subject,x.faculty,x.classroom]); const subjects=data.subjects.map(x=>[x.code,x.name,x.department,x.faculty]); const notices=data.notices.map(x=>[x.title,x.category,new Date(x.published_at).toLocaleDateString(),x.description]);
  let content;
  if(title==='User Management')content=<Table columns={['Name','Email','Primary role','Portal roles','Created']} rows={(data.users||[]).map(x=>[x.full_name,x.email,x.role,(x.roles||[]).join(', '),new Date(x.created_at).toLocaleDateString()])}/>;
  else if(title==='Timetable Management'&&role==='academic_coordinator')content=<TimetableManagement rows={data.timetable} reload={reload}/>;
  else if(title.includes('Timetable'))content=<Table columns={['Day','Time','Subject','Faculty','Room']} rows={timetable}/>;
  else if(title.includes('Subject')||title.includes('Academic Management')||title.includes('Class Information'))content=<Table columns={['Code','Subject','Department','Faculty']} rows={subjects}/>;
  else if(title==='Attendance'&&role==='student')content=<Table columns={['Code','Subject','Classes','Present','Percentage']} rows={(student.attendance?.subjects||[]).map(x=>[x.code,x.name,x.classes,x.present,`${x.percentage}%`])}/>;
  else if(title.includes('Assignments'))content=<Table columns={['Title','Subject','Deadline','Status']} rows={(student.assignments||[]).map(x=>[x.title,x.subject,new Date(x.deadline).toLocaleString(),x.status])}/>;
  else if(title==='Leave Requests'&&role==='student')content=<LeaveRequests/>;
  else if(title.includes('Marks')||title.includes('Results'))content=<Table columns={['Code','Subject','Assessment','Score','Percentage']} rows={(student.marks||[]).map(x=>[x.code,x.name,x.examination_type,`${x.score}/${x.maximum_score}`,`${x.percentage}%`])}/>;
  else if(title.includes('Calendar'))content=<AcademicCalendarManagement role={role}/>;
  else if(title.includes('Exam')||title.includes('Schedule'))content=<ExamScheduleManagement role={role}/>;
